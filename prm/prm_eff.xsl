...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:variable name="vv" select="root/tbl[2]/row/@eff" />
                    ...
                    <g id="poly">
                        <xsl:variable name="points">
                            <xsl:for-each select="tbl[2]/row">
                                ...
                                <xsl:variable name="y" select="format-number($ph * (1 - (@eff - $vinf) div $vrng),'0.0')"/>
                                <xsl:choose>
                    ...
                    <g id="line">
                        <xsl:variable name="line1">
                            <xsl:for-each select="tbl[2]/row">
                                ...
                                <xsl:variable name="y" select="format-number($ph * (1 - (@eff - $vinf) div $vrng),'0.00')"/>
                                <xsl:choose>
                                    <xsl:when test="position()=1">
                                        <xsl:text>M </xsl:text>
                                        <xsl:value-of select="$x"/>
                    ...
                    <g id="line">
                        <xsl:variable name="line2">
                            <xsl:for-each select="tbl[2]/row[@yr &gt;= 2010]">
                                ...
                                <xsl:variable name="x" select="format-number($pw * (@yr - $tmin) div $trng,'0.00')"/>
                                <!--<xsl:variable name="y" select="@*[$j + 3]"/>-->
                                <xsl:variable name="y" select="format-number($ph * (1 - (@prm2_eff - $vinf) div $vrng),'0.00')"/>
                                <xsl:choose>
                                    <xsl:when test="position()=1">
                                        <xsl:text>M </xsl:text>
                                        <xsl:value-of select="$x"/>
